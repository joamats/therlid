DROP TABLE IF EXISTS `spring-carving-271217.test.cci`;

CREATE TABLE `spring-carving-271217.test.cci` AS
...
WITH t1 AS (
  SELECT
    ...
    CASE
      ...
      WHEN CAST(s.age AS numeric) BETWEEN 70 AND 79 THEN 3
      WHEN CAST(s.age AS numeric) BETWEEN 60 AND 69 THEN 2
      WHEN CAST(s.age AS numeric) BETWEEN 50 AND 59 THEN 1
      ELSE 0
    END AS age_score_charlson
  FROM
    `aiwonglab.eicu_crd_ii_v0_1_0.patient` s
  LEFT JOIN
    `aiwonglab.eicu_crd_ii_v0_1_0.pasthistory` ph
  ON
